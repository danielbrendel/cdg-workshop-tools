/*
	...
	Tool: Shotgun (developed by Daniel Brendel)
	Version: 0.1
...
void CDG_API_Trigger(const Vector& in vAtPos)
{
	...
	Vector vLineDest1, vLineDest2, vLineDest3;
	
	int iMaxShotDistance = 20;
	
	for (int i = 0; i < 10; i++) {
		CDecalEntity@ obj = CDecalEntity();
		Ent_SpawnEntity(@obj, Vector(vAtPos[0] + (Util_Random(1, iMaxShotDistance * 2) - iMaxShotDistance), vAtPos[1] + (Util_Random(1, iMaxShotDistance * 2) - iMaxShotDistance)));
	}
