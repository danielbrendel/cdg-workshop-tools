/*
	...
	Tool: Wolfdragon (developed by Daniel Brendel)
	Version: 0.1
...
const int C_WOLFDRAGON_REACT_RANGE = 400;
const int C_WOLFDRAGON_ATTACK_RANGE = 320;
const float C_WOLFDRAGON_DEFAULT_SPEED = 3.5;
class CWolfdragon : IScriptedEntity
{
	...
	void OnSpawn(const Vector& in vec)
	{
		...
		this.m_hMove = R_LoadSprite(g_szToolPath + "wolfdragon.png", 5, 391, 243, 1, false);
		this.m_hAttackSound = S_QuerySound(g_szToolPath + "laser.wav");
		...
		BoundingBox bbox;
		bbox.Alloc();
		bbox.AddBBoxItem(Vector(25, 25), this.m_vecSize);
		this.m_oModel.Alloc();
	...
	void OnRelease()
	{
		...
		Ent_SpawnEntity(@obj, Vector(this.m_vecPos[0] + 150, this.m_vecPos[1] + 100));
		
		SoundHandle hSplash = S_QuerySound(g_szToolPath + "sound\\hc_splash.wav");
	...
	void OnDrawOnTop()
	{
		...
		R_DrawSprite(this.m_hMove, this.m_vecPos, this.m_iSpriteIndex, this.m_fRotation, Vector(-1, -1), 0.5f, 0.5f, bCustomColor, Color(red, green, blue, alpha));
	}
