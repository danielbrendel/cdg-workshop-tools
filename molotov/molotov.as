/*
	...
	Tool: Molotov (developed by Daniel Brendel)
	Version: 0.1
...
class CMolotovEntity : IScriptedEntity
{
	Vector m_vecPos;
	Model m_oModel;
	Timer m_tmrLifeTime;
	Timer m_tmrMovement;
	SpriteHandle m_hSprite;
	SoundHandle m_hBreak;
	SoundHandle m_hFlame;
	float m_fRotation;
	float m_fDisplayRot;
	float m_fSpeed;
	...
	void OnSpawn(const Vector& in vec)
	{
		...
		this.m_hSprite = R_LoadSprite(g_szToolPath + "molotov.png", 1, 32, 32, 1, false);
		this.m_hBreak = S_QuerySound(g_szToolPath + "break.wav");
		this.m_hFlame = S_QuerySound(g_szToolPath + "explode.wav");
	...
	void OnRelease()
	{
		S_PlaySound(this.m_hBreak, 10);
		S_PlaySound(this.m_hFlame, 10);
...
{
	CMolotovEntity@ obj = CMolotovEntity();
