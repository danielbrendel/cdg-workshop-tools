/*
	...
	Tool: Mine (developed by Daniel Brendel)
	Version: 0.1
...
class CExplosion : IScriptedEntity
{
	...
	void OnSpawn(const Vector& in vec)
	{
		...
		BoundingBox bbox;
		bbox.Alloc();
		bbox.AddBBoxItem(Vector(-50, -50), Vector(200, 200));
		this.m_oModel.Alloc();
	...
	void OnDrawOnTop()
	{
		R_DrawSprite(this.m_hSprite, this.m_vecPos, this.m_iFrameCount, 0.0, Vector(-1, -1), 4.0, 4.0, false, Color(0, 0, 0, 0));
	}
...
class CMine : IScriptedEntity
{
			...
			if ((@pEntity != null) && (@pEntity != @this) && (pEntity.GetName().length() > 0) && (pEntity.IsDamageable() != DAMAGEABLE_NO)) { //Validate entity
				...
				if ((this.m_vecPos.Distance(pEntity.GetPosition()) < MAX_CHECK_RANGE)) {
					if (!this.m_bInitiated) {
						...
						this.m_tmrBlink.SetDelay(250);
						this.m_tmrBlink.Reset();
						this.m_tmrBlink.SetActive(true);
					}
				}
			}
		}
	...
	CMine()
    {
		...
		this.m_tmrBlink.SetActive(false);
    }
	...
	{
		this.m_vecPos = vec;
		this.m_hSprite1 = R_LoadSprite(g_szToolPath + "mine_grey.png", 1, 48, 35, 1, true);
		this.m_hSprite2 = R_LoadSprite(g_szToolPath + "mine_green.png", 1, 48, 35, 1, true);
		this.m_hTick = S_QuerySound(g_szToolPath + "tick.wav");
	...
	void OnProcess()
	{
		...
		if (this.m_tmrBlink.IsActive()) {
			this.m_tmrBlink.Update();
			if (this.m_tmrBlink.IsElapsed()) {
				this.m_tmrBlink.Reset();
				
				this.m_iBlinkCount++;
				S_PlaySound(this.m_hTick, 10);
			}
		}
	}
	...
	void OnDraw()
	{
		if ((this.m_tmrBlink.IsActive()) && ((this.m_iBlinkCount % 2) == 0)) {
			R_DrawSprite(this.m_hSprite2, this.m_vecPos, 0, 0.0, Vector(-1, -1), 0.0, 0.0, false, Color(0, 0, 0, 0));
		} else {
			R_DrawSprite(this.m_hSprite1, this.m_vecPos, 0, 0.0, Vector(-1, -1), 0.0, 0.0, false, Color(0, 0, 0, 0));
		}	
	}
